...
#wd <- "~./Input_data"
wd <- "D:/Dropbox (ScreenForBio)/Projects/Thanh_occupancy/Handle_lumping_data/docs/manuscript/Submission/CSP/Input_data"
setwd(wd)
...
# Load the effort
effort <- as.matrix(read.csv("Covariates/ObsCovs/Effort/effort.csv", row.names=1))
effort <- effort[order(row.names(effort)),]


# Load site as detection covariate
sites_detection <- as.matrix(read.csv("Covariates/ObsCovs/Site/site_det.csv", row.names=1))
sites_detection <- sites_detection[order(row.names(sites_detection)),]



# Load the detection history that was generated by camtrapr
detecthist <- as.matrix(read.csv("Detection_history/detection_history.csv",row.names = 1))
...
#####  Plot response curves for all covariates #####

## Plot response curves of random effect model
...
for(i in 1:length(unique(nd1$site))) {
  nd1$include[nd1$site == unique(nd1$site)[i]] <- nd1[nd1$site == unique(nd1$site)[i],]$E <= max(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd1$site)[i], "E"]) &
    nd1[nd1$site == unique(nd1$site)[i],]$E >= min(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd1$site)[i], "E"])

  nd2$include[nd2$site == unique(nd2$site)[i]] <- nd2[nd2$site == unique(nd2$site)[i],]$L <= max(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd2$site)[i], "L"]) &
    nd2[nd2$site == unique(nd2$site)[i],]$L >= min(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd2$site)[i], "L"])

  nd3$include[nd3$site == unique(nd3$site)[i]] <- nd3[nd3$site == unique(nd3$site)[i],]$V <= max(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd3$site)[i], "V"]) &
    nd3[nd3$site == unique(nd3$site)[i],]$V >= min(Site_Covs_all_short[Site_Covs_all_short$site == unique(nd3$site)[i], "V"])
}

nd1 <- nd1[nd1$include, -ncol(nd1)]
nd2 <- nd2[nd2$include, -ncol(nd2)]
nd3 <- nd3[nd3$include, -ncol(nd3)]
...
prediction1$site <- factor(prediction1$site , levels = c("PMNP", "PDNR", "XeSap", "Saola", "BachMa", "STNR"),
                           labels = c("Pu Mat NP", "Phong Dien NR", "Xe Sap/Palé",   "Saola NRs",   "Bach Ma NP",  "Song Thanh NP"))

# get mean response
coef(o_mul_04)
ranef(o_mul_04, "state")
coefs_mean_effect <- sapply(ranef(o_mul_04, "state")$site, mean)    # mean parameter effect

# predictions for mean effects
logit_psi_mean_E <- coefs_mean_effect[1] + unique(nd1$E) * coefs_mean_effect["E"]
logit_psi_mean_L <- coefs_mean_effect[1] + unique(nd2$L) * coefs_mean_effect["L"]
logit_psi_mean_V <- coefs_mean_effect[1] + unique(nd3$V) * coefs_mean_effect["V"]

psi_mean_E <- exp(logit_psi_mean_E) / (exp(logit_psi_mean_E) + 1)
psi_mean_L <- exp(logit_psi_mean_L) / (exp(logit_psi_mean_L) + 1)
psi_mean_V <- exp(logit_psi_mean_V) / (exp(logit_psi_mean_V) + 1)


df_tmp <- data.frame(Predicted = c(psi_mean_E, psi_mean_L, psi_mean_V),
                     SD = NA, lower = NA, upper = NA,
                     site = "Mean effect",
                     L = c(rep(0, times = length(psi_mean_E)), unique(nd2$L), rep(0, times = length(psi_mean_V))),
                     E = c(unique(nd1$E), rep(0, times = length(psi_mean_L)), rep(0, times = length(psi_mean_V))),
                     V = c(rep(0, times = length(psi_mean_E)), rep(0, times = length(psi_mean_L)), unique(nd3$V)),
                     covariate = rep(c("Elevation", "Least cost path", "Village density"), each = length(psi_mean_E)))

df_tmp$ele <-  df_tmp$E * attr(scale(Site_Covs_all$ele), "scaled:scale") + attr(scale(Site_Covs_all$ele), "scaled:center")
df_tmp$LCP <-  df_tmp$L * attr(scale(Site_Covs_all$LCP), "scaled:scale") + attr(scale(Site_Covs_all$LCP), "scaled:center")
df_tmp$vill <- df_tmp$V * attr(scale(Site_Covs_all$vill), "scaled:scale") + attr(scale(Site_Covs_all$vill), "scaled:center")

df_tmp$covariate_value <- c(df_tmp$ele [1:100], df_tmp$LCP[101:200], df_tmp$vill[201:300])

prediction1_comb <- rbind(prediction1, df_tmp)

# coda::plot(model_mcmc_list, ask = T)
model_mcmc_list <- As.mcmc.list(o_mul_04@stanfit)
model_mcmc_comb <- do.call(rbind, model_mcmc_list)
colnames(model_mcmc_comb) <- o_mul_04@stanfit@sim$fnames_oi

mean(rowMeans(model_mcmc_comb[,grep("b_state[L", colnames(model_mcmc_comb), fixed = T)]))
quantile(rowMeans(model_mcmc_comb[,grep("b_state[L", colnames(model_mcmc_comb), fixed = T)]), 0.025)
quantile(rowMeans(model_mcmc_comb[,grep("b_state[L", colnames(model_mcmc_comb), fixed = T)]), 0.975)

# plot responses for all covariates covariates
plot(hcl_palettes(type = "qualitative"))
palette_to_use <- "Dynamic"
cbbPalette6 <- c("#E69F00", "#56B4E9", "#009E73", "#0072B2", "#D55E00", "#CC79A7")
alpha_ribbon_lines <- 0.9
lty <- 3

response_random <- ggplot(prediction1, mapping = aes(x = covariate_value, y = Predicted, color = site, fill = site)) +
  labs(x = "Covariate value", y = "") +

  geom_line(data = df_tmp, aes(x = covariate_value, y = Predicted, color = site), color = "black", inherit.aes = FALSE,
            linetype = 2, size = 1.2, alpha = 0.7, show.legend = F) +

  geom_ribbon(mapping = aes(ymin = lower,  ymax = upper, color = NULL), alpha = 0.04) +
  geom_line(mapping = aes(y = lower), alpha = alpha_ribbon_lines, linetype = lty) +
  geom_line(mapping = aes(y = upper), alpha = alpha_ribbon_lines, linetype = lty) +
  geom_line(size = 2, alpha = 1) +
  scale_y_continuous(position = "right", breaks = c(0, 0.25, 0.5, 0.75, 1), limits = c(0, 1)) +
  facet_wrap(~covariate, nrow = 1, scales = "free_x")+
  theme_bw() +
  scale_colour_manual(values = cbbPalette6) +
  theme(strip.text.x = element_text(size = 30, colour = "black", angle = 00),
        axis.text.x=element_text(size=21, angle = 0.5,vjust=0.3,hjust=0.5),
...
## Plot response curves of fixed effect
#### Response curves for fixed effect in which all data were pooled together
###$ Covarites values are original
# make newdata (vary one covariate, other covariates = 0 (at their mean))
nd1 <- expand.grid(list(site = unique(Site_Covs_all_short$site),
                        L = 0,
                        E = seq(min(Site_Covs_all_short$E), max(Site_Covs_all_short$E), length.out = 100),
...
prediction2$site <- factor(prediction2$site,
                           levels = c("PMNP", "PDNR",   "XeSap",   "Saola",  "BachMa",   "STNR"),
                           labels = c("Pu Mat NP", "Phong Dien NR", "Xe Sap/Palé",   "Saola NRs",   "Bach Ma NP",  "Song Thanh NP"))
...
response_fixed <- ggplot(prediction2, mapping = aes(x = covariate_value, y = Predicted, color = site, fill = site)) +
  geom_ribbon(mapping = aes(ymin = lower,  ymax = upper, color = NULL), alpha = 0.04) +
  geom_line(mapping = aes(y = lower), alpha = alpha_ribbon_lines, linetype = lty) +
  geom_line(mapping = aes(y = upper), alpha = alpha_ribbon_lines, linetype = lty) +
  geom_line(size = 2, alpha = 1) +
...
#### Plot covariate effects by reserve ##########
...
all_names_subset <- all_names[grep("b_state[L ", all_names, fixed = TRUE)]
...
#THIS IS IMPORTANT:We calculate 75% BCI.
between75 <- which(between(0, tmp5$lower75, tmp5$upper75))        # group 3: 75%BCI overlaps 0         -> grey


tmp5$level <- NA
tmp5$level [notbetween95] <- 1
tmp5$level [between95] <- 2
tmp5$level [between75] <- 3     # here we use 75% BCI. You could also use 50% if you want
...
p1 <- ggplot(tmp5, mapping = aes(y = median, x = area, group=covariate)) +
  ...
  theme(axis.title.x=element_blank(),
        ...
        axis.title.y=element_blank(),
        axis.text.x=element_text(size=21, angle = 0,vjust=0.3,hjust=1),
